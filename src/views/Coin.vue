<template>
  <div class="coins">
    ...
    <table>
      ...
      <tbody>
        <tr v-for="(value, key) in coins" :key="key" :class="{ rise: value.tradePrice - value.prevTradePrice > 0, fall: value.tradePrice - value.prevTradePrice < 0 }">
          <td>{{key}}</td>
          <td>{{value.tradePrice}}</td>
          <td>{{changeDirection(value.change)}}{{value.changePrice}}</td>
          <td>{{value.changeRate}}</td>
          <td>{{value.openingPrice}}</td>
...
@keyframes rise {
  0% {
    bacground-color: rgba(0, 255, 0, 0);
  }
  50% {
